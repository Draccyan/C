...
Game::Game(short int _width, short int _height, std::string _language, sf::RenderWindow &_window1) : window1(_window1)
{
    ...
    lifeText.setColor(sf::Color(0, 255, 0));
    lifeText.setPosition(width/3, 0);
    ...
    ///cargar textura de nave enemiga
...
for(int i=0;i<1;i++)
{
    ...
    ///llamamos al ciclo infinito del juego
    gameLoop();

    /*for(auto entity:entities){ ///en esta parte auto le asigna el tipo *Asteroid
        delete (entity);
    }*/
    ///creamos un vector de entidades(clase que heredan la mayoria de nuestros objetos)
...
void Game::gameLoop()
{
    while (window1.isOpen() && !gameOver)
    {
        ...
        lifeText.setString("life: " + std::to_string(ship->getLifePoints()));
        ...
        ///condicion para limitar la cantidad de asteroides
        ///y el tiempo de reaparicion
        if (time % 150 == 0 && asteroidNumber <= 4)
        {
            ///generacion de asteroides
            for (int i = 0; i < 3; i++)
            {
                asteroidNumber++;
        ...
        ///condicion de reaparicion de enemigos
        if (time % 300 == 0)
        {
        ...
        ///actualizador del vector de entidades, tanto posicion como animacion
        ///llamamos a la funcion que verifica las colisiones
        for (auto i = entities.begin(); i != entities.end();)
        {
            Entity *e = *i;
            ...
            ///preguntamos si la vida del objeto llamado asteroide es cero, entonces suma puntos
            ///y se genera la animacion de explosion
            if (e->getLife() == 0)
            {
    ...
    for (auto b : entities)
    {
        ...
        if (a->getName().compare("player") == 0)
        {
            if (b->getName().compare("asteroid") == 0)
            {
                if (isCollide(a, b))
                {
                    a->setLifePoints(a->getLifePoints()-15);
                    b->setLife(0);
                }
            }
            else if (b->getName().compare("enemy") == 0)
            {
                if (isCollide(a, b))
                {
                    a->setLifePoints(a->getLifePoints()-25);
                    b->setLife(0);
                }
            }
            else if (b->getName().compare("enemyBullet") == 0)
            {
                if (isCollide(a, b))
                {
                    a->setLifePoints(a->getLifePoints()-5);
                    b->setLife(0);
                }
            }
...
///aca chequeamos todos los eventos de la ventana, mouse y teclado
